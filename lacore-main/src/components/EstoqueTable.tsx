...
import { Search, Plus, Package, Edit2, Camera, Sparkles, ChevronDown, ChevronUp, History } from 'lucide-react'
...
export default function EstoqueTable() {
  ...
  const agrupar = (): Grupo[] => {
    ...
    prods.forEach(p => {
      const k = `${p.brand}||${p.model}||${p.storage || ''}||${p.condition || ''}`
              ...
              {grupos.map(g => (
                <>
                  <tr key={g.key} className="cursor-pointer" onClick={() => setExpanded(expanded === g.key ? null : g.key)}>
                    <td>
                      <div className="flex items-center gap-2">
                        {g.fotos[0] ? <img src={g.fotos[0].url} className="w-8 h-8 rounded-lg object-cover flex-shrink-0" /> : <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0"><Package size={13} className="text-gray-600" /></div>}
                        <div><p className="text-white font-semibold text-sm">{g.brand} {g.model}</p><p className="text-gray-500 text-xs">{g.qtd > 1 ? `${g.qtd} unidades` : '1 unidade'}</p></div>
                      </div>
                    </td>
            ...
            <div className="flex gap-2 flex-wrap">
              {editModal.grupo?.fotos.map((f, i) => (
                <div key={i} className="relative">
                  <img src={f.url} className="w-20 h-20 rounded-lg object-cover border border-white/10" />
                </div>
              ))}
