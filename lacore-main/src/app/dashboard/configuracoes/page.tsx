...
export default function ConfiguracoesPage() {
  ...
  const [status, setStatus] = useState<{ openai: boolean; manus: boolean; gemini: boolean } | null>(null)
      ...
      <div className="card">
        ...
        {loading ? <div className="flex gap-3">{[1,2,3].map(i => <div key={i} className="h-12 flex-1 rounded-lg bg-white/5 animate-pulse" />)}</div> : (
          <div className="grid grid-cols-3 gap-3">
            {keys.map(k => (
              <div key={k.id} className={`p-3 rounded-lg border ${status?.[k.id] ? 'bg-green-500/8 border-green-500/20' : 'bg-white/3 border-white/8'}`}>
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs text-gray-400">{k.label.split(' ')[0]}</p>
                  {status?.[k.id] ? <CheckCircle size={13} className="text-green-400" /> : <XCircle size={13} className="text-gray-600" />}
                </div>
                <p className={`text-xs font-semibold ${status?.[k.id] ? 'text-green-300' : 'text-gray-600'}`}>{status?.[k.id] ? '✓ Configurada' : 'Não configurada'}</p>
                {status?.[k.id] && <button onClick={() => removeKey(k.id)} className="mt-1.5 text-[10px] text-red-400 hover:text-red-300 flex items-center gap-0.5"><Trash2 size={9} />Remover</button>}
              </div>
            ))}
          </div>
        )}
