...
export default function EstoqueTable() {
  ...
  const buscarSpecs = async () => {
    ...
    const s = r.specs || {}
    setEditForm(f => ({ ...f, processor: s.processor || f.processor, ram: s.ram || f.ram, camera_main: s.camera_main || f.camera_main, camera_front: s.camera_front || f.camera_front, battery: s.battery || f.battery, screen: s.screen || f.screen, nfc: s.nfc ?? f.nfc, charging: s.charging !== '-' ? (s.charging || f.charging) : f.charging, connectivity: s.connectivity !== '-' ? (s.connectivity || f.connectivity) : f.connectivity, os: s.os !== '-' ? (s.os || f.os) : f.os, dimensions: s.dimensions !== '-' ? (s.dimensions || f.dimensions) : f.dimensions }))
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const grupo = editModal.grupo!
      const specFields = {
        processor: editForm.processor || null,
        ram: editForm.ram || null,
        camera_main: editForm.camera_main || null,
        camera_front: editForm.camera_front || null,
        battery: editForm.battery || null,
        screen: editForm.screen || null,
        nfc: editForm.nfc || false,
        charging: editForm.charging || null,
        connectivity: editForm.connectivity || null,
        os: editForm.os || null,
      ...
      await sb.from('products').update({
        ...
        catalog_highlight: editForm.catalog_highlight,
        catalog_description: editForm.catalog_description,
            ...
            <tbody>
              {filtrados.map(p => (
                <tr key={p.id}>
                  ...
                  {isAdmin() && <td className="hide-mobile font-mono text-xs text-gray-400">{fR(p.cost_brl_unit || 0)}</td>}
                  <td className="font-mono font-semibold text-cyan-300">{fR(p.price_current || 0)}</td>
                  <td className="hide-mobile text-gray-500 text-xs">{fD(p.date_added || '')}</td>
                  {isAdmin() && (
                    <td>
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[['Processador', 'processor'], ['RAM', 'ram'], ['Bateria (mAh)', 'battery'], ['Carregamento (ex: 67W)', 'charging'], ['Tela (pol. + tipo + Hz)', 'screen'], ['Conectividade (ex: 5G, Dual SIM)', 'connectivity'], ['Câmera Traseira', 'camera_main'], ['Câmera Frontal', 'camera_front'], ['Sistema Operacional', 'os'], ['Dimensões / Peso', 'dimensions']].map(([label, key]) => (
                <div key={key}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: 'var(--text-2)' }}>
                  <input type="checkbox" checked={editForm.nfc || false} onChange={e => setEditForm(f => ({ ...f, nfc: e.target.checked }))} style={{ flexShrink: 0 }} />
                  <span>Possui NFC</span>
                </label>
                <span className={`badge ${editForm.nfc ? 'badge-green' : 'badge-gray'}`}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2">
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '8px 10px', borderRadius: 8, background: editForm.catalog_visible !== false ? 'rgba(37,99,235,.08)' : 'var(--bg-3)', border: `1px solid ${editForm.catalog_visible !== false ? 'rgba(37,99,235,.25)' : 'var(--border-1)'}`, transition: 'all .2s' }}>
                  <input type="checkbox" checked={editForm.catalog_visible !== false} onChange={e => setEditForm(f => ({ ...f, catalog_visible: e.target.checked }))} style={{ flexShrink: 0 }} />
                  <div>
                    <p style={{ fontSize: 12.5, fontWeight: 600, color: editForm.catalog_visible !== false ? 'var(--accent)' : 'var(--text-3)', display: 'flex', alignItems: 'center', gap: 5 }}>
                      {editForm.catalog_visible !== false ? <><Eye size={12} />Visível no Catálogo</> : <><EyeOff size={12} />Oculto do Catálogo</>}
                    </p>
                    <p style={{ fontSize: 10.5, color: 'var(--text-4)', marginTop: 1 }}>
                      {editForm.catalog_visible !== false ? 'Aparece para clientes — DISPONÍVEL ou ESGOTADO conforme estoque' : 'Não aparece no catálogo público'}
                    </p>
                  </div>
                </label>
              </div>
