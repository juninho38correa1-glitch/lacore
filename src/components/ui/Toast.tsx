...
function ToastItem({ item, onRemove }: { item: ToastItem; onRemove: () => void }) {
  ...
  return (
    <div onClick={() => { setVisible(false); setTimeout(onRemove, 250) }} style={{
      ...
      {/* Ícone */}
      {(() => { const Icon = ICONS[item.type]; return (
        <div style={{ width: 24, height: 24, borderRadius: 99, background: `rgba(${c.icon === '#10B981' ? '16,185,129' : c.icon === '#EF4444' ? '239,68,68' : c.icon === '#3B82F6' ? '59,130,246' : '245,158,11'},.15)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: c.icon, flexShrink: 0 }}>
