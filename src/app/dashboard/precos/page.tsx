          ...
          {showDrop && cidadesFiltradas.length > 0 && (
            <div style={{ position: 'absolute', zIndex: 20, width: '100%', marginTop: 4, background: 'var(--bg-1)', border: '1px solid var(--border-2)', borderRadius: 8, overflow: 'hidden', boxShadow: 'var(--shadow-lg)', maxHeight: 220, overflowY: 'auto' }}>
              {cidadesFiltradas.map((c, i) => (
                <button key={i} onClick={() => selCidade(c)} className="prod-select-btn" style={{ width: '100%', textAlign: 'left', padding: '9px 14px', fontSize: 13, color: 'var(--text-2)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>{c.nome}</span>
