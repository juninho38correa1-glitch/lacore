...
export default function PublicacoesPage() {
  ...
  useEffect(() => {
    sb.from('products').select('id,brand,model,color,storage,condition,price_current,cost_brl_unit,processor,ram,camera_main,camera_front,battery,screen').eq('status', 'ATIVO').order('brand').then(({ data }) => {
      // Agrupar por brand+model+storage — pega o representante de cada grupo
      const map: Record<string, Product> = {}
      ;(data || []).forEach((p: Product) => {
        const k = `${p.brand}||${p.model}||${p.storage || ''}`
        if (!map[k]) map[k] = p // guarda apenas o primeiro do grupo
      })
      setProds(Object.values(map) as Product[])
    })
  }, [])
          ...
          <div className="card space-y-4">
            <div>
              ...
              <select value={sel} onChange={e => setSel(e.target.value)} className="text-sm">
                ...
                {prods.map(p => <option key={p.id} value={p.id}>{p.brand} {p.model}{p.storage ? ` ${p.storage}` : ''}</option>)}
              </select>
            </div>
            {prod && (
