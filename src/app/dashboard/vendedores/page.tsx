    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {loading ? (
        ...
        <div className="grid-vend">
          ...
          {shown.map(v => (
            <div key={v.id} className="card card-glow card-border-anim" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'novo' ? 'Novo Vendedor' : `Editar — ${(modal as User)?.name}`} size="sm">
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>
            <label className="label">{modal !== 'novo' ? 'Senha (vazio = manter)' : 'Senha *'}</label>
